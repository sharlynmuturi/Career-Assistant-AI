...
- Extract the most likely values even if not explicitly labeled.
- Infer the role if it is implied (e.g., "Data Scientist (Asset Management)" - "Data Scientist").
- If experience is not explicitly stated, infer from context (e.g., "1-3 years").
- Extract skills from requirements, tools, and responsibilities.
- Only return null if absolutely no reasonable inference can be made.
...
job_data = {
    "role": job_data.get("role") or "Unknown Role",
...
with st.spinner("Extracting job data and running analysis..."):
    ...
    if job_data and (job_data.get("description") or job_data.get("skills")) and not top_semantic_projects.empty:
